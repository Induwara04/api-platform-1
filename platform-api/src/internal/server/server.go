...
func StartPlatformAPIServer(cfg *config.Server, slogger *slog.Logger) (*Server, error) {
	...
	artifactImportService := service.NewArtifactImportService(apiRepo, llmProviderRepo, llmTemplateRepo, llmProxyRepo, mcpProxyRepo, artifactRepo, deploymentRepo, gatewayRepo, projectRepo, cfg, slogger)
	...
	internalGatewayHandler := handler.NewGatewayInternalAPIHandler(gatewayService, internalGatewayService, artifactImportService, slogger)

...
func setupImportTest(t *testing.T, syncMetadata bool) *importTestDeps {
	t.Helper()
	...
	if _, err := db.Exec(`INSERT INTO gateways (uuid, organization_uuid, name, display_name, description, vhost, sync_metadata, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))`,
		importTestGatewayID, importTestOrgID, "gw1", "Gateway 1", "", "gw.example.com", syncMetadata); err != nil {
	...
	svc := NewArtifactImportService(apiRepo, providerRepo, templateRepo, proxyRepo, mcpProxyRepo,
		artifactRepo, deploymentRepo, gatewayRepo, projectRepo, cfg, logger)
...
func TestArtifactImport_CPArtifactMetadataNotOverwritten(t *testing.T) {
	d := setupImportTest(t, true) // even with sync_metadata, CP origin must be protected
...
func TestArtifactImport_DPArtifactMetadataUpdatedWhenSyncMetadata(t *testing.T) {
	d := setupImportTest(t, true)

	const id = "66666666-6666-6666-6666-666666666666"
	// First import creates the DP artifact.
	resp, err := d.svc.Import(importTestOrgID, importTestGatewayID, restImportRequest(id, "dp-api", "First Name"))
	if err != nil {
		t.Fatalf("first import: %v", err)
	}
	cpID := resp.ID
	// Second import (same handle) from the metadata-owning gateway updates the display name.
	if _, err := d.svc.Import(importTestOrgID, importTestGatewayID, restImportRequest(id, "dp-api", "Second Name")); err != nil {
		t.Fatalf("second import: %v", err)
	}

	art, _ := d.artifactRepo.GetByUUID(cpID, importTestOrgID)
	if art == nil || art.Name != "Second Name" {
		t.Errorf("DP artifact name = %v, want updated to 'Second Name' when sync_metadata=true", art)
	}
}

func TestArtifactImport_DPArtifactMetadataNotUpdatedWhenNotSyncing(t *testing.T) {
	d := setupImportTest(t, false) // gateway does not own metadata
	...
	if art == nil || art.Name != "First Name" {
		t.Errorf("DP artifact name = %v, want unchanged 'First Name' when sync_metadata=false", art)
	}
}
...
func TestArtifactImport_UndeployThenRedeploy(t *testing.T) {
	d := setupImportTest(t, true) // sync_metadata = true: this gateway owns the metadata

	const id = "ab000000-0000-0000-0000-000000000001"

	// 1. Initial deploy.
	resp, err := d.svc.Import(importTestOrgID, importTestGatewayID, restImportRequest(id, "rd-api", "Original Name"))
	if err != nil {
	...
	//    (status DEPLOYED), and updates metadata because this gateway syncs metadata.
	redeploy := restImportRequest("ab000000-0000-0000-0000-00000000aaaa", "rd-api", "Updated Name")
	...
	if art2.Name != "Updated Name" {
		t.Errorf("redeploy with sync_metadata=true did not update metadata name: got %q", art2.Name)
	}
}

func TestArtifactImport_RedeployNoMetadataUpdateWhenNotSyncing(t *testing.T) {
	d := setupImportTest(t, false) // gateway does NOT own metadata

	const id = "ab000000-0000-0000-0000-000000000002"
	resp, err := d.svc.Import(importTestOrgID, importTestGatewayID, restImportRequest(id, "rd2-api", "Original Name"))
	if err != nil {
		t.Fatalf("deploy: %v", err)
	}
	cpID := resp.ID
	// Re-deploy with a different display name; metadata must NOT change.
	if _, err := d.svc.Import(importTestOrgID, importTestGatewayID, restImportRequest(id, "rd2-api", "Updated Name")); err != nil {
		t.Fatalf("redeploy: %v", err)
	}
	art, _ := d.artifactRepo.GetByUUID(cpID, importTestOrgID)
	if art == nil || art.Name != "Original Name" {
		t.Errorf("metadata changed despite sync_metadata=false: got %v", art)
	}
}

...
func TestLiftLLMPolicies_RoundTrip(t *testing.T) {
	...
	// Import decode (same as the importer's decodeSpec) + reverse lift.
	var cfg model.LLMProviderConfig
	if err := decodeSpec(spec, &cfg); err != nil {
		t.Fatalf("decodeSpec: %v", err)
	}
	sec, rl, remaining := liftLLMPolicies(cfg.Policies)
	...
	// --- Genuine policy preserved; system policies stripped ---
	if len(remaining) != 1 || remaining[0].Name != "custom-guardrail" {
		t.Errorf("remaining policies = %+v, want only [custom-guardrail]", remaining)
	}
	for _, p := range remaining {
		switch p.Name {
		case importPolicyAPIKeyAuth, importPolicyTokenRateLimit, importPolicyAdvancedRateLimit,
			importPolicyCostRateLimit, importPolicyLLMCost:
			t.Errorf("system policy %q leaked into remaining policies", p.Name)
		}
	}
}

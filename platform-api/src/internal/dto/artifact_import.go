...
type ImportGatewayArtifactRequest struct {
	// ID is the data-plane (gateway) artifact UUID. The control plane does NOT reuse it
	// as the CP artifact UUID — it mints its own and returns it in the response. This
	// value is informational (traceability); artifacts are matched by handle.
...
// ImportGatewayArtifactResponse is the generic response for an imported artifact.
type ImportGatewayArtifactResponse struct {
	...
	Origin          string     `json:"origin"` // always "DP" for imported artifacts
	CreatedAt       time.Time  `json:"createdAt"`
	UpdatedAt       time.Time  `json:"updatedAt"`
	DeployedAt      *time.Time `json:"deployedAt,omitempty"`

...
func (r *GatewayRepo) Create(gateway *model.Gateway) error {
	...
	query := `
		INSERT INTO gateways (uuid, organization_uuid, name, display_name, description, properties, vhost, is_critical,
		                      gateway_functionality_type, version, is_active, sync_metadata, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`
	_, err := r.db.Exec(r.db.Rebind(query), gateway.ID, gateway.OrganizationID, gateway.Name, gateway.DisplayName,
		...
		gateway.IsActive, gateway.SyncMetadata, gateway.CreatedAt, gateway.UpdatedAt)
	return err
}
...
func (r *GatewayRepo) GetByUUID(gatewayId string) (*model.Gateway, error) {
	...
	err := r.db.QueryRow(r.db.Rebind(query), gatewayId).Scan(
		...
		&gateway.IsCritical, &gateway.FunctionalityType, &gateway.Version, &gateway.IsActive, &gateway.SyncMetadata, &gateway.CreatedAt, &gateway.UpdatedAt)
	if err != nil {
...
func (r *GatewayRepo) UpdateGateway(gateway *model.Gateway) error {
	...
	query := `
		UPDATE gateways
		SET display_name = ?, description = ?, is_critical = ?, properties = ?, sync_metadata = ?, updated_at = ?
		WHERE uuid = ?
	`
	_, err := r.db.Exec(r.db.Rebind(query), gateway.DisplayName, gateway.Description, gateway.IsCritical, propertiesJSON, gateway.SyncMetadata, gateway.UpdatedAt, gateway.ID)
	return err
}
